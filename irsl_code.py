...
def placeObj(baseObj, moveObj, basePoint, movePoint):
    cds = baseObj.get_transformed(basePoint)
    inv = movePoint.inverse_transformation()
    cds.transform(inv)
    moveObj.newcoords(cds)

def dataOf(data, tgt):
    return [ d for d in data if d[5] == tgt ]

def _clusterData(data):
    res = {}
    for d in data:
        if d[5] in res:
            res[ d[5] ].append(d)
...
def dumpData(topDir='/tmp/', draw=False): ## di, lp, parts_list
    ...
    for pname in parts_list:
        ...
        # print('\n\n')
        # aa = input()
    with open(f'{topDir}data.pkl', 'wb') as f:
        pickle.dump(data, f)
